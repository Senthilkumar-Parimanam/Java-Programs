...
public class ParallelProgProcessor extends RecursiveTask<List<String>>
{
   ...
   public ParallelProgProcessor(String path, String extension)
   {
      this.path = path;
   ...
   protected List<String> compute()
   {
      ...
      //ParallelProgProcessor tasks to store the subtasks that are going to process the subfolders stored in the folder
      List<ParallelProgProcessor> tasks = new ArrayList<ParallelProgProcessor>();
      //Get the content of the folder.
      File file = new File(path);
      File content[] = file.listFiles();
      //For each element in the folder, if there is a subfolder, create a new ParallelProgProcessor object 
      ...
      if (content != null)
      {
         for (int i = 0; i < content.length; i++)
         {
            ...
            {
               ParallelProgProcessor task = new ParallelProgProcessor(content[i].getAbsolutePath(), extension);
               task.fork();
               tasks.add(task);
            }
      ...
      //If the list of the ParallelProgProcessor subtasks has more than 50 elements, 
   ...
   private void addResultsFromTasks(List<String> list, List<ParallelProgProcessor> tasks)
   {
      for (ParallelProgProcessor item : tasks)
      {
